...
namespace SatelliteGameJam.Networking.Voice
{
    ...
    /// - Ground Control: Can only transmit while interacting with transmission console
    ...
    public class VoiceChatP2P : MonoBehaviour
    {
        ...
        private bool ShouldRecordVoice()
        {
            ...
            // Ground Control: only record while at transmission console
            if (localState.Role == PlayerRole.GroundControl)
            {
                return VoiceSessionManager.Instance?.IsLocalPlayerAtConsole ?? false;
            }

            // Space Station: always recording (routing controls who receives)
        ...
        private bool ShouldSendVoiceTo(PlayerState localState, SteamId targetId)
        {
            ...
            if (localState.Role == PlayerRole.GroundControl)
            {
                // Ground Control can only transmit when at transmission console.
                ...
                // While at console, Ground can send to Ground and Space players.
                if (targetState.Role == PlayerRole.GroundControl)
                {
                    if (debugLogging) Debug.Log($"[VoiceChatP2P] Ground->Ground (at console): sending to {targetId}");
                    return true;
                }

                if (targetState.Role == PlayerRole.SpaceStation)
                {
                    if (debugLogging) Debug.Log($"[VoiceChatP2P] Ground->Space (at console): sending to {targetId}");
                    return true;
                }
        ...
        private void HandleIncomingVoicePacket(byte[] data, int length)
        {
            ...
            VoiceRemotePlayer remotePlayer = CreateRemoteVoicePlayer(senderId);
        ...
        {
            return VoiceSessionManager.Instance != null
                ? VoiceSessionManager.Instance.GetOrCreateVoiceRemotePlayer(senderId)
                : null;
        }
